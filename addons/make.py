...
def AddLinks():
	for i in dict.keys():
		try:
			dict[i].append(GetOriginalLink(dict2[i]))
		except:
			pass
...
• If a control is offscreen, tones will not be played.""", "https://addons.nvda-project.org/files/get.php?file=objLoc"],
	"BGT Lullaby":["unknown", """This addon causes NVDA to automatically go to sleep when running BGT games.
Any speech sent to NVDA by the game will still be spoken, as will any dialogs (input fields and messages).""", "https://addons.nvda-project.org/files/get.php?file=bgt"],
	"Enhanced Aria":["2017.4 to 2019.2", """This addon allows you to customize which aria landmarks are reported by NVDA when you browse the Internet.
...
Contact info
This addon has been developed by Jose Manuel Delicado. If you want to contact me, send an e-mail to jm.delicado@nvda.es, or open an issue on GitHub at https://github.com/jmdaweb/enhancedAria""", "https://addons.nvda-project.org/files/get.php?file=earia"],
...
Pressing SPL Assistant, Shift+F8 will present a dialog to let you request playlist transcripts in numerous formats, including in a plain text format, an HTML table or a list.""", "https://addons.nvda-project.org/files/get.php?file=spl"],
	"Tone Master":["unknown", """Welcome to Tone Master! I've created this little NVDA add-on just for fun, but also for you to have fun while using it.
...
. 1Each line in .tdf file must contain three parameters separated by colon (:). The first parameter is tone pitch, the second parameter is tone duration, and the third one is the time of silence between each tone. All three parameters are necessary to specify, otherwise Tone Master will not be able to play your tone data.
. 2Pitch and duration parameters must be specified as signed integers, and the silence must be specified as floating point real value.
. 3A hash sign (#) at the beginning of any line in .tdf file will be treeted as a comment and will be ignored by Tone Master.
